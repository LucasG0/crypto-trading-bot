...
class SStochastic(BotStrategy):
    def __init__(self, chart_m5, chart_m15, chart_d1):
        super(SStochastic,self).__init__()
    ...
    def cond_stochastic(self,candlestick):
        stochastic = self.indicators.stochastic(self.prices,self.low,self.high,14)
        if (stochastic < 20):
            self.inf20 = True
            return False
        else:
    ...
    def open_position(self,candlestick):
        st = self.cond_stochastic(candlestick)
